...
fig2  <-  function(mcmcMat = mmfit$BUGSoutput$sims.matrix) {
	...
	plot(x, y, axes=FALSE, type='n', xlab='Status', ylab=substitute('C'[CO[2]]*' (% air saturation)'), xlim=c(0.5,2.5), ylim=c(3.5, 16.5))
	box()
	axis(1, at=c(1,2), labels=c('Invasive', 'Native'))
	axis(2, at=seq(4, 16, 4), las=1)
	...
	par(mai=c(1.02,0.22,0.82,1.20))
	plot(x, y, axes=FALSE, type='n', xlab='Shape', ylab='', xlim=c(0.5,2.5), ylim=c(3.5, 16.5))
	box()
	axis(1, at=c(1,2), labels=c('Erect', 'Flat'))
	axis(2, las=1, at=seq(4, 16, 4), labels=NA)
	points(x, y, pch=16, col=transparentColor(rep(c('tomato', 'dodgerblue2'), each=nrow(mcmcMat)), 0.1), cex=1.3)
...
fig3  <-  function(mcmcMat = mmfit$BUGSoutput$sims.matrix) {
	...
	plot(x, y, axes=FALSE, type='n', xlab='Status', ylab=substitute('C'[CO[2]]*' (% air saturation)'), xlim=c(0.5,2.5), ylim=c(5, 20), xpd=NA)
